...
export const AddAuction = ({ setAuction }) => {
  ...
  const submitForm = async (e) => {
    ...
    console.log(currentDate.getHours() + parseInt(itemDuration.current.value))
    let dueDate = currentDate.setHours(
      currentDate.getHours() + parseInt(itemDuration.current.value)
    );
    if(parseFloat(itemDuration.current.value) %1 !=0){
      console.log(currentDate.getMinutes())
      console.log(parseFloat(itemDuration.current.value)%1)
      dueDate = currentDate.setMinutes(currentDate.getMinutes()+ ((parseFloat(itemDuration.current.value)%1)*60)%60)
      dueDate = currentDate.setHours(
        currentDate.getHours() + ((parseFloat(itemDuration.current.value)%1)*60)/60)
            ...
            <Row>
              ...
              <Col>
                <Form.Group>
                  ...
                  <Form.Control type="number" step="0.01" required ref={itemDuration} />
                </Form.Group>
              </Col>
            </Row>
            <Row>
